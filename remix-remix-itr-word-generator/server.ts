...
Support all ITR forms: ITR-1, ITR-2, ITR-3, ITR-4, ITR-5, ITR-6, ITR-7, and ITR-V Acknowledgement.`;
    ...
    for (const modelName of candidateModels) {
      // Try model with exponential backoff on 503/429
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          ...
          // If model is not found, break to next candidate model immediately
          if (errMsg.includes('404') || errMsg.includes('NOT_FOUND') || errMsg.includes('no longer available') || errMsg.includes('is not found')) {
            break;
          }

          // If 503 / 429 / high demand, wait with backoff before retry or cascading to next model
          if (attempt < 2) {
