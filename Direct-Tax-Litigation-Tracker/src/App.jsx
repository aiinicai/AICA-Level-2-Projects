...
async function getJSON(key, shared) {
  const storageKey = (shared ? "litigation_shared:" : "litigation:") + key;
...
function makeSeedDb() {
  ...
  const users = [
    { user_id: "admin", name: "Admin (Consultant)", email: "admin@taxconsult.in", password: "Admin@123", role: "Admin", group_id: null, status: "Active", created_at: todayISO(-120) },
    { user_id: "sharma_user", name: "Rohit Sharma", email: "rohit@sharmagroup.in", password: "User@123", role: "GroupUser", group_id: "GRP001", status: "Active", created_at: todayISO(-90) },
    { user_id: "nexa_user", name: "Priya Menon", email: "priya@nexaindustries.in", password: "User@123", role: "GroupUser", group_id: "GRP002", status: "Active", created_at: todayISO(-60) }
  ];
  const invitedEmails = [
...
export default function LitigationTracker() {
  ...
  async function init() {
    let d, p, s;
    try { d = await getJSON("app-data", true); } catch (e) { d = makeSeedDb(); await setJSON("app-data", d, true).catch(() => {}); }
    try { p = await getJSON("proceedings-data", true); } catch (e) { p = makeSeedProceedings(); await setJSON("proceedings-data", p, true).catch(() => {}); }
    try { s = await getJSON("session", false); } catch (e) { s = null; }
    setDb(d); setProc(p); setSessionUserId(s ? s.userId : null);
    setLoading(false);
  }

  const persistDb = useCallback(async (next) => {
    setDb(next);
    try { await setJSON("app-data", next, true); } catch (e) { console.error("storage error", e); }
  }, []);
  const persistProc = useCallback(async (next) => {
    setProc(next);
    try { await setJSON("proceedings-data", next, true); } catch (e) { console.error("storage error", e); }
  }, []);
  const logAuth = useCallback(async (entry) => {
    try {
      let arr;
      try { arr = await getJSON("auth-logs", true); } catch (e) { arr = []; }
      arr.push(entry);
      if (arr.length > 300) arr = arr.slice(-300);
      await setJSON("auth-logs", arr, true);
    } catch (e) { /* best effort */ }
  }, []);
  ...
  async function doLogin(userId, password) {
    ...
    const success = !!u && u.password === password && u.status === "Active";
    await logAuth({ user_id: userId, event: "login", success, at: new Date().toISOString(), status_note: u ? (u.status !== "Active" ? "inactive account" : "") : "unknown user" });
    if (!u) { notify("No account with that User ID.", "error"); return; }
    if (u.status !== "Active") { notify("This account has been deactivated by the Admin.", "error"); return; }
    if (u.password !== password) { notify("Incorrect password.", "error"); return; }
    setSessionUserId(u.user_id);
    await setJSON("session", { userId: u.user_id }, false).catch(() => {});
    notify(`Welcome back, ${u.name.split(" ")[0]}.`);
  }

  async function doLogout() {
    setSessionUserId(null);
    await setJSON("session", {}, false).catch(() => {});
  }

  async function doSignup({ name, email, userId, password }) {
    if (db.users.some((u) => u.user_id === userId)) { notify("That User ID is already taken.", "error"); return; }
    const isFirstEver = db.users.length === 0;
    let role = "GroupUser", group_id = null, invite = null;
    if (!isFirstEver) {
      invite = db.invited_emails.find((i) => i.email.toLowerCase() === email.toLowerCase() && !i.used);
      if (!invite) { notify("This email hasn't been invited by the Admin. Ask them to add it first.", "error"); return; }
      group_id = invite.group_id;
    } else {
      role = "Admin";
    }
    const newUser = { user_id: userId, name, email, password, role, group_id, status: "Active", created_at: todayISO() };
    const nextDb = { ...db, users: [...db.users, newUser] };
    if (invite) nextDb.invited_emails = db.invited_emails.map((i) => (i.email === invite.email ? { ...i, used: true } : i));
    await persistDb(nextDb);
    await logAuth({ user_id: userId, event: "signup", success: true, at: new Date().toISOString(), status_note: role === "Admin" ? "bootstrap admin" : "invited signup" });
    setSessionUserId(userId);
    await setJSON("session", { userId }, false).catch(() => {});
    notify(role === "Admin" ? "Admin account created — you're the first user in this workspace." : "Account created. Welcome aboard!");
  }
...
const NAV = [
  ...
  { key: "companies", label: "Companies", icon: Building2, adminOnly: true },
  { key: "users", label: "Users", icon: UsersIcon, adminOnly: true },
  { key: "master", label: "Master Lists", icon: ListChecks, adminOnly: true }
];
...
function Dashboard({ db, proceedings, isAdmin }) {
  ...
  const totalDemand = ongoing.reduce((s, p) => s + (Number(p.tax_demand_amount) || 0), 0);
  ...
  return (
    <div>
      <Card className="lt-filterbar">
        ...
        <input className="lt-input" placeholder="Assessment Year e.g. 2023-24" value={filters.ay} onChange={(e) => setFilters((f) => ({ ...f, ay: e.target.value }))} style={{ maxWidth: 190 }} />
        ...
        {Object.values(filters).some(Boolean) && <button className="lt-link-btn" onClick={() => setFilters({ group_id: "", company_id: "", ay: "", nature: "", status: "", stage: "", act: "" })}>Clear filters</button>}
      </Card>

      <div className="lt-stat-grid">
        <StatCard icon={<FileText size={18} color="#fff" />} label="Total proceedings" value={filtered.length} tone="var(--ink)" />
        <StatCard icon={<Clock size={18} color="#fff" />} label="Ongoing" value={ongoing.length} sub={`${closed.length} closed · ${completed.length} completed`} tone="var(--amber)" />
        <StatCard icon={<IndianRupee size={18} color="#fff" />} label="Demand outstanding (ongoing)" value={fmtINR(totalDemand)} tone="var(--rust)" />
        <StatCard icon={<AlertTriangle size={18} color="#fff" />} label="Hearings in next 7 days" value={next7.length} sub={`${next30.length} within 30 days`} tone="var(--emerald)" />
      </div>

      <div className="lt-chart-grid">
        {isAdmin && (
          <Card>
            <h4>Group-wise proceedings</h4>
        ...
        <Card>
          <h4>Company-wise proceedings</h4>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={byCompany} layout="vertical" margin={{ left: 10 }}>
              ...
              <Tooltip />
              <Bar dataKey="value" fill="#5B7A99" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Card>
        <Card>
          <h4>Status-wise split</h4>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={byStatus} dataKey="value" nameKey="name" outerRadius={80} innerRadius={45}>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={byStage}>
              ...
              <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
              <Tooltip />
              <Bar dataKey="value" fill="#A8823C" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Card>
        <Card>
          <h4>Nature of proceeding</h4>
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie data={byNature} dataKey="value" nameKey="name" outerRadius={85}>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={byYear}>
              ...
              <Tooltip formatter={(v, n) => (n === "demand" ? fmtINR(v) : v)} />
              <Legend wrapperStyle={{ fontSize: 12 }} />
              <Line yAxisId="l" type="monotone" dataKey="count" name="Proceedings" stroke="#14213D" strokeWidth={2} />
              <Line yAxisId="r" type="monotone" dataKey="demand" name="demand" stroke="#A83B2E" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </Card>
      </div>

      <div className="lt-alert-grid">
        <Card>
          <h4><Clock size={15} style={{ verticalAlign: -2 }} /> Hearings in the next 7 days</h4>
          {next7.length === 0 ? <Empty text="Nothing on the calendar in the next 7 days." /> : (
            <table className="lt-mini-table">
              <thead><tr><th>Entity</th><th>Issue</th><th>Next hearing</th><th>Stage</th></tr></thead>
              <tbody>
                {next7.sort((a, b) => a.next_hearing_date.localeCompare(b.next_hearing_date)).map((p) => (
                  <tr key={p.proceeding_id}>
                    ...
                    <td className="lt-truncate">{p.issue_involved}</td>
                    <td>{fmtDate(p.next_hearing_date)}</td>
        ...
        <Card>
          <h4><AlertTriangle size={15} style={{ verticalAlign: -2, color: "var(--rust)" }} /> Overdue on statutory timeline</h4>
          {overdue.length === 0 ? <Empty text="No open proceedings are past their due date." /> : (
            <table className="lt-mini-table">
              <thead><tr><th>Entity</th><th>Issue</th><th>Due date</th><th>Days overdue</th></tr></thead>
              <tbody>
                {overdue.sort((a, b) => a.proceeding_timeline_due_date.localeCompare(b.proceeding_timeline_due_date)).map((p) => (
                  <tr key={p.proceeding_id}>
                    <td>{companyName(p.company_id)}</td>
                    <td className="lt-truncate">{p.issue_involved}</td>
                    <td>{fmtDate(p.proceeding_timeline_due_date)}</td>
                    <td style={{ color: "var(--rust)", fontWeight: 600 }}>{Math.abs(daysFromToday(p.proceeding_timeline_due_date))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ProceedingsScreen({ db, proc, persistProc, currentUser, isAdmin, scoped, notify }) {
  ...
  function exportCSV() {
    const cols = ["s_no", "company_id", "group_id", "financial_year", "assessment_year", "applicable_act", "relevant_section", "nature_of_proceeding", "proceeding_status", "stage_of_proceeding", "issue_involved", "tax_demand_amount", "demand_type", "last_hearing_date", "next_hearing_date", "proceeding_timeline_due_date", "consultant_name", "remarks"];
  ...
  function exportExcel() {
    const rows = filtered.map((p) => ({
      ...
      "Issue": p.issue_involved, "Demand (₹)": p.tax_demand_amount, "Demand Type": p.demand_type,
      ...
      <Card style={{ overflowX: "auto", padding: 0 }}>
        <table className="lt-table">
          ...
          <tbody>
            {pageRows.map((p) => {
              ...
              return (
                <tr key={p.proceeding_id}>
                  ...
                  <td>{fmtINR(p.tax_demand_amount)}</td>
            ...
            {pageRows.length === 0 && <tr><td colSpan={11}><Empty text="No proceedings match these filters." /></td></tr>}
          </tbody>
        </table>
      </Card>
  ...
  return (
    <Modal title={isNew ? "Add proceeding" : "Edit proceeding"} onClose={onClose} wide>
      <div className="lt-form-grid">
        ...
        <Field label="Relevant Section"><input className="lt-input" value={form.relevant_section} onChange={set("relevant_section")} /></Field>
        ...
        <Field label="Demand type"><Select value={form.demand_type} onChange={(v) => setForm((f) => ({ ...f, demand_type: v }))} options={DEMAND_TYPES} /></Field>
        <Field label="Tax demand (₹)"><input className="lt-input" type="number" value={form.tax_demand_amount} onChange={set("tax_demand_amount")} /></Field>
...
function UsersScreen({ db, persistDb, currentUser, notify }) {
  ...
  async function resetPassword(u, pwd) {
    await persistDb({ ...db, users: db.users.map((x) => (x.user_id === u.user_id ? { ...x, password: pwd } : x)) });
