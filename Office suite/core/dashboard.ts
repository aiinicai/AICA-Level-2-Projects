...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const modulesList = document.getElementById('modules-list') as HTMLUListElement;
  ...
  const activeToolTitle = document.getElementById('active-tool-title') as HTMLElement;
  const activeToolBadge = document.getElementById('active-tool-badge') as HTMLElement;
  const btnRefresh = document.getElementById('btn-refresh') as HTMLButtonElement;
  const btnOpenTab = document.getElementById('btn-open-tab') as HTMLButtonElement;
  ...
  let currentActiveModule: ModuleInfo | null = null;
  let currentTheme: 'dark' | 'light' = 'dark';

  // --- Theme Management ---
  function initTheme() {
    const savedTheme = localStorage.getItem('office_suite_theme') as 'dark' | 'light' | null;
  ...
  function notifyIframeTheme(theme: 'dark' | 'light') {
    if (toolFrame && toolFrame.contentWindow) {
      try {
        toolFrame.contentWindow.postMessage({ type: 'THEME_CHANGE', theme }, '*');
      } catch (err) {
  ...
  function renderModuleList(modules: ModuleInfo[]) {
  ...
  function selectModule(mod: ModuleInfo, element: HTMLElement) {
