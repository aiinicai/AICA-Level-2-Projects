...
================================================================================
 ...
 ----------
   ...
   2. Run: python MF_XRay_V1.py
   3. The application window opens
   4. Click "Load Demo Portfolio" on the Dashboard to explore every module
...
# =============================================================================
APP_NAME = "MF X-Ray"
APP_FULL_TITLE = "MF X-RAY — AI-Powered Mutual Fund Portfolio Intelligence & Risk Analysis System"
APP_SUBTITLE = "AI-Powered Mutual Fund Portfolio Intelligence & Risk Analysis System"
APP_VERSION = "V1.0"
DEMO_DATA_LABEL = "DEMO DATA — NOT LIVE MARKET DATA"

DISCLAIMER_SHORT = (
    "MF X-Ray is an educational and analytical portfolio-intelligence tool. "
    "It is not investment advice and does not constitute a recommendation to buy, hold, "
    "sell or switch any mutual fund or security."
)

DISCLAIMER_FULL = (
    "MF X-Ray is an educational and analytical portfolio-intelligence tool. "
    "It is not investment advice and does not constitute a recommendation to buy, hold, "
    "sell or switch any mutual fund or security. Users should independently evaluate "
...
DEMO_DATA_DISCLAIMER = (
    ...
    "illustrative/sample data and should not be treated as current market information."
)
...
def get_fund_monthly_returns(fund_name: str) -> pd.Series:
    if fund_name not in FUND_NAV_HISTORY:
        if fund_name in FUNDS:
            fund = FUNDS[fund_name]
            months = ASSUMPTIONS["nav_history_months"]
            end_date = fund.portfolio_date if hasattr(fund, "portfolio_date") and fund.portfolio_date else date.today()
            mu_m = (1.0 + (getattr(fund, "cagr_3y_pct", 12.0) or 12.0) / 100.0) ** (1.0 / 12.0) - 1.0
            sigma_m = _RISK_MONTHLY_SIGMA.get(getattr(fund, "risk_level", "Moderate"), 0.04)
            rng = np.random.RandomState(stable_seed(fund_name, 7))
            shocks = rng.normal(mu_m, sigma_m, size=months)
            rel = np.cumprod(1.0 + shocks[::-1])[::-1]
            nav_series = getattr(fund, "nav", 10.0) / rel
            dates = [end_date - timedelta(days=30 * i) for i in reversed(range(months))]
            FUND_NAV_HISTORY[fund_name] = pd.Series(nav_series, index=dates)
        else:
...
def calc_fund_risk_table(fund_names, risk_free_rate_pct) -> pd.DataFrame:
    ...
    for name in fund_names:
        ...
        nav = FUND_NAV_HISTORY.get(name, pd.Series(dtype=float))
        rows.append({
            "Fund": name,
...
def answer_question(question: str, metrics: dict, snapshots: dict = None) -> str:
    """A deterministic, rule-based portfolio intelligence Q&A layer over the
    ALREADY CALCULATED metrics dict. It never invents information or gives
    unsolicited buy/sell advice: if the data needed to answer is not available,
    it states so explicitly without hallucinating."""
    ...
    top_sector_pct = sector_df.iloc[0]["Effective Exposure %"] if not sector_df.empty else 0.0
    ...
    # 1. Chartered Accountant perspective / Summary
    if "like a ca" in q or "chartered accountant" in q or "audit" in q:
        return (
            f"CA Portfolio Intelligence Summary:\n"
            f"1. Capital Base: Total investment of {format_inr(metrics['total_investment'])} deployed across {num_funds} scheme(s).\n"
            f"2. Asset Allocation: Equity {format_pct(metrics['weighted_equity_pct'])}, Debt {format_pct(metrics['weighted_debt_pct'])}, Cash {format_pct(metrics['weighted_cash_pct'])}.\n"
            f"3. Single-Stock Risk: Look-through exposure reveals top single-stock concentration of {format_pct(conc.get('largest_pct', 0))} in '{conc.get('largest_company')}'.\n"
            f"4. Sectoral Exposure: Primary sector exposure is '{top_sector_name}' at {format_pct(top_sector_pct)}.\n"
            f"5. Scheme Overlap: Average pairwise overlap stands at {format_pct(avg_overlap)}, indicating that scheme duplication dilutes intended diversification.\n"
            f"6. Portfolio Health Score: {health['total']}/100 ({health_score_risk_bucket(health['total'])} risk profile). "
            f"Note: Review common underlying holdings rather than adding additional funds."
        )

    if any(k in q for k in ["explain", "summar"]):
        return generate_ai_narrative(metrics)

    # 2. Company lookup: "Which funds contain Company Alpha / Northbridge Bank?"
    for company_name in list(COMPANY_SECTOR_MAP.keys()) + (exposure["Company"].tolist() if not exposure.empty else []):
        if company_name.lower() in q:
            # Found matching company
            fund_matches = []
            for fname in alloc["Fund"].tolist():
                if fname in FUNDS and company_name in FUNDS[fname].holdings:
                    fund_matches.append(f"'{fname}' ({format_pct(FUNDS[fname].holdings[company_name])} weight)")
            exp_row = exposure[exposure["Company"] == company_name] if not exposure.empty else None
            eff_info = ""
            if exp_row is not None and not exp_row.empty:
                r = exp_row.iloc[0]
                eff_info = f" In your overall portfolio, your effective look-through exposure to {company_name} is {format_pct(r['Effective Exposure %'])} ({format_inr(r['Exposure Amount'])})."
            if fund_matches:
                return f"'{company_name}' is held by {len(fund_matches)} of your funds: {', '.join(fund_matches)}.{eff_info}"
            else:
                return f"'{company_name}' is not currently held in any of your selected mutual funds."

    # Check for general "contain" / "which fund holds"
    if any(k in q for k in ["contain", "which fund holds", "who holds"]):
        # Find which company might be mentioned
        for c in COMPANY_SECTOR_MAP:
            words = c.lower().split()
            if any(w in q for w in words if len(w) > 4):
                fund_matches = [f"'{fname}' ({format_pct(FUNDS[fname].holdings[c])})" for fname in alloc["Fund"] if fname in FUNDS and c in FUNDS[fname].holdings]
                if fund_matches:
                    return f"'{c}' is held by: {', '.join(fund_matches)}."
    ...
    if "overlap" in q and any(k in q for k in ["most", "which two", "highest", "top"]):
        ...
        return f"'{a}' and '{b}' overlap the most, sharing {format_pct(pct)} of their portfolios (MF X-Ray Overlap Methodology)."

    if "overlap" in q:
        return f"The average pairwise overlap across your held funds is {format_pct(avg_overlap)}. Top overlapping pair is '{overlap_pairs[0][0]}' and '{overlap_pairs[0][1]}' at {format_pct(overlap_pairs[0][2])}."
    ...
    if "sector" in q and any(k in q for k in ["highest", "most", "top", "largest"]):
        ...
        return f"'{top['Sector']}' has the highest effective exposure at {format_pct(top['Effective Exposure %'])} ({format_inr(top['Exposure Amount'])}), contributed by {top['Funds Contributing']} fund(s)."

    # 5. Why is portfolio concentrated?
    if "why" in q and ("concentrat" in q or "high" in q):
        return (
            f"Your portfolio is concentrated primarily because: "
            f"(1) Common holdings across schemes: your largest single holding '{conc.get('largest_company')}' represents "
            f"{format_pct(conc.get('largest_pct', 0))} effective exposure due to presence in multiple funds; "
            f"(2) Sector concentration: '{top_sector_name}' accounts for {format_pct(top_sector_pct)} of total exposure; and "
            f"(3) Top 5 holdings together represent {format_pct(conc.get('top5_pct', 0))} of your portfolio."
        )

    # 6. Highest exposure / top stocks
    if "highest exposure" in q or ("top" in q and "compan" in q) or ("top" in q and "stock" in q) or ("highest" in q and "stock" in q):
    ...
    if "chang" in q or "previous" in q or "snapshot" in q:
        if snapshots and len(snapshots) >= 2:
            snap_names = list(snapshots.keys())
            return f"You have {len(snapshots)} snapshots saved ({', '.join(snap_names)}). Go to the 'Portfolio Changes' module to see exact stock-level and sector-level percentage point changes."
        return ("To track what changed, save your portfolio as a snapshot in the 'Portfolio Changes' module. "
                "Once you save at least two snapshots, MF X-Ray will compute the exact percentage-point changes across stocks and sectors.")

    if "health" in q or "score" in q:
        return (f"Your MF X-Ray Portfolio Health Score is {health['total']}/100 "
                f"(risk bucket: {health_score_risk_bucket(health['total'])}). "
                f"Components: Diversification ({health['components']['diversification']}/20), "
                f"Overlap ({health['components']['overlap']}/20), "
                f"Stock Concentration ({health['components']['stock_concentration']}/20), "
                f"Sector Concentration ({health['components']['sector_concentration']}/15), "
                f"Volatility ({health['components']['volatility']}/15), "
                f"Drawdown ({health['components']['drawdown']}/10).")

    return "Insufficient data available to answer this question. Try asking: 'Which companies have the highest exposure?', 'Which funds overlap the most?', 'Which sector has the highest exposure?', 'Why is my portfolio concentrated?', or 'Explain my portfolio like a CA.'"
...
class MFXRayApp:
    ...
    def _setup_window(self):
        self.root.title(f"{APP_NAME} {APP_VERSION} — ICAI AI Level 2 Capstone Project")
        try:
    ...
    def _build_header(self):
        ...
        Label(left, text=f"{APP_NAME} {APP_VERSION}", font=("Segoe UI", 20, "bold"),
              fg="white", bg=COLORS["header_bg"]).pack(anchor=W)
        Label(left, text="AI-Powered Mutual Fund Portfolio Intelligence & Risk Analysis System (ICAI AI Level 2)", font=("Segoe UI", 10),
    ...
    def _build_body(self):
        ...
        self.footer_label = Label(self.footer, text=f"{APP_NAME} {APP_VERSION}   |   {DEMO_DATA_LABEL}   |   {DISCLAIMER_FULL}", font=("Segoe UI", 8),
    ...
    def add_fund_to_portfolio(self, fund_name, amount_str, method, date_str):
        try:
            ...
            existing = [e for e in self.portfolio.entries if e.fund_name == fund_name]
            if existing:
                existing[0].amount = amount
                existing[0].method = method
                existing[0].inv_date = inv_date
                messagebox.showinfo("Fund Updated", f"Updated investment for '{fund_name}' to {format_inr(amount)}.")
            else:
    ...
    def _build_dashboard(self, parent, metrics):
        ...
        kpis = [
            ...
            ("EQUITY EXPOSURE", format_pct(metrics["weighted_equity_pct"], 1), "Weighted across funds", COLORS["text_dark"]),
            ("DEBT EXPOSURE", format_pct(metrics["weighted_debt_pct"], 1), "Weighted across funds", COLORS["text_dark"]),
        ]
        ...
        kpis2 = [
            ...
            ("PORTFOLIO OVERLAP", format_pct(avg_overlap), "Average of every fund pair", COLORS["amber"] if avg_overlap >= THRESHOLDS["high_overlap_pct"] else COLORS["text_dark"]),
            ("PORTFOLIO HEALTH SCORE", f"{health['total']} / 100", f"Risk Level: {risk_bucket}", RISK_BUCKET_COLORS.get(risk_bucket)),
        ]
        self._kpi_row(parent, kpis2)

        # Health Score Breakdown Card (Section 19)
        health_card = Frame(parent, bg=COLORS["card_bg"], highlightbackground=COLORS["border"], highlightthickness=1)
        health_card.pack(fill=X, padx=24, pady=(6, 4))
        Label(health_card, text=f"Portfolio Health Score: {health['total']} / 100 ({risk_bucket} Risk)",
              font=("Segoe UI", 12, "bold"), bg=COLORS["card_bg"]).pack(anchor=W, padx=14, pady=(10, 2))
        Label(health_card, text="MF X-Ray analytical score based on 6 transparent, weighted risk factors (not an official rating):",
              font=("Segoe UI", 8, "italic"), fg=COLORS["text_muted"], bg=COLORS["card_bg"]).pack(anchor=W, padx=14, pady=(0, 6))

        h_grid = Frame(health_card, bg=COLORS["card_bg"])
        h_grid.pack(fill=X, padx=14, pady=(0, 10))
        comp_items = [
            ("Diversification", f"{health['components']['diversification']} / 20", "Fund count & allocation balance"),
            ("Fund Overlap", f"{health['components']['overlap']} / 20", "Common holding penalty"),
            ("Stock Concentration", f"{health['components']['stock_concentration']} / 20", "Top-5 single-stock exposure"),
            ("Sector Concentration", f"{health['components']['sector_concentration']} / 15", "Single-sector exposure"),
            ("Volatility", f"{health['components']['volatility']} / 15", "Annualised portfolio variance"),
            ("Drawdown", f"{health['components']['drawdown']} / 10", "Historical peak-to-trough decline"),
        ]
        for idx, (label, score_str, desc) in enumerate(comp_items):
            box = Frame(h_grid, bg=COLORS["bg"], highlightbackground=COLORS["border"], highlightthickness=1)
            box.grid(row=0, column=idx, padx=4, pady=4, sticky="nsew")
            h_grid.columnconfigure(idx, weight=1)
            Label(box, text=label, font=("Segoe UI", 9, "bold"), bg=COLORS["bg"], fg=COLORS["text_dark"]).pack(anchor=W, padx=8, pady=(6, 2))
            Label(box, text=score_str, font=("Segoe UI", 11, "bold"), bg=COLORS["bg"], fg=COLORS["accent_dark"]).pack(anchor=W, padx=8)
            Label(box, text=desc, font=("Segoe UI", 7), bg=COLORS["bg"], fg=COLORS["text_muted"]).pack(anchor=W, padx=8, pady=(0, 6))

        cols_frame = Frame(parent, bg=COLORS["bg"])
        cols_frame.pack(fill=X, padx=16, pady=4)
    ...
    def _build_my_portfolio(self, parent, metrics):
        ...
        ttk.Button(form, text="Add / Update Fund", style="Accent.TButton",
        ...
        if metrics["has_portfolio"]:
            ...
            left_card = self._card_in(cols_frame)
            Label(left_card, text="Current Holdings & Allocation", font=("Segoe UI", 12, "bold"),
                  bg=COLORS["card_bg"]).pack(anchor=W, padx=14, pady=(10, 0))
            self._make_table(left_card, alloc, height=8,
            ...
            remove_row = Frame(left_card, bg=COLORS["card_bg"])
            ...
            ttk.Button(remove_row, text="Remove", command=lambda: self.remove_fund_from_portfolio(remove_var.get())).pack(side=LEFT, padx=4)
            ttk.Button(remove_row, text="Clear Entire Portfolio", command=self.clear_portfolio).pack(side=LEFT, padx=10)

            right_card = self._card_in(cols_frame)
            Label(right_card, text="Allocation Donut Chart (Section 12)", font=("Segoe UI", 12, "bold"),
                  bg=COLORS["card_bg"]).pack(anchor=W, padx=14, pady=(10, 0))

            def draw_donut_alloc(ax):
                ax.pie(alloc["Allocation %"], labels=alloc["Fund"], autopct="%1.1f%%",
                       startangle=90, wedgeprops={"width": 0.42}, textprops={"fontsize": 8})
                ax.set_title("Fund-wise Asset Distribution", fontsize=10)

            self._embed_figure(right_card, draw_donut_alloc, figsize=(5.0, 3.8))
    ...
    def _build_sector_analysis(self, parent, metrics):
        ...
        cols_frame.pack(fill=BOTH, expand=True, padx=16)
        left = self._card_in(cols_frame)
        Label(left, text="Sector Exposure Table", font=("Segoe UI", 12, "bold"), bg=COLORS["card_bg"]).pack(anchor=W, padx=14, pady=(10, 0))
        self._make_table(left, sector_df, height=10,
                          col_formats={"Exposure Amount": format_inr, "Effective Exposure %": format_pct})

        right = self._card_in(cols_frame)
        Label(right, text="Sector Allocation (Donut Chart)", font=("Segoe UI", 12, "bold"), bg=COLORS["card_bg"]).pack(anchor=W, padx=14, pady=(10, 0))
        ...
        self._embed_figure(right, draw_donut, figsize=(5.0, 4.0))

        # Bar chart card (Section 16: Bar chart + Donut/pie chart)
        bar_card = self._card(parent)
        Label(bar_card, text="Sector Concentration Ranking (Bar Chart)", font=("Segoe UI", 12, "bold"),
              bg=COLORS["card_bg"]).pack(anchor=W, padx=14, pady=(10, 0))

        def draw_bar(ax):
            top_s = sector_df.sort_values("Effective Exposure %", ascending=True)
            colors = [COLORS["accent"] if v < THRESHOLDS["high_sector_exposure_pct"] else COLORS["amber"]
                      for v in top_s["Effective Exposure %"]]
            bars = ax.barh(top_s["Sector"], top_s["Effective Exposure %"], color=colors, height=0.55)
            ax.axvline(x=THRESHOLDS["high_sector_exposure_pct"], color=COLORS["red"], linestyle="--",
                       linewidth=1.2, label=f"Threshold ({THRESHOLDS['high_sector_exposure_pct']}%)")
            ax.set_xlabel("Effective Exposure %", fontsize=8)
            ax.set_title("Effective Sector Exposure Ranking with High Concentration Threshold", fontsize=10)
            ax.tick_params(axis="both", labelsize=8)
            ax.legend(loc="lower right", fontsize=8)
            for bar in bars:
                w = bar.get_width()
                ax.text(w + 0.3, bar.get_y() + bar.get_height() / 2, f"{w:.1f}%", va="center", fontsize=7)
            ax.set_xlim(0, max(float(sector_df["Effective Exposure %"].max()) * 1.25, 35))

        self._embed_figure(bar_card, draw_bar, figsize=(9.5, 3.5))

        self._methodology_box(parent,
            f"MF X-Ray Analytical Threshold for 'high sector concentration': {format_pct(THRESHOLDS['high_sector_exposure_pct'])} "
            f"of the portfolio's effective exposure. Sectors are assigned using this application's sample "
    ...
    def _build_ai_insights(self, parent, metrics):
        ...
        def ask(*_):
            ans = answer_question(q_var.get(), metrics, self.snapshots)
            answer_label.configure(text="A: " + ans)
            answer_label.pack(anchor=W, padx=14, pady=(8, 8))

        ttk.Button(row, text="Ask MF X-Ray", style="Accent.TButton", command=ask).pack(side=LEFT)
        ...
        examples = [
            ...
            "What changed from the previous portfolio?",
        ]
        ex_frame = Frame(ask_card, bg=COLORS["card_bg"])
        ex_frame.pack(fill=X, padx=14, pady=(6, 14))
        Label(ex_frame, text="Quick Questions:", bg=COLORS["card_bg"], fg=COLORS["text_muted"],
    ...
    def _build_data_management(self, parent, metrics):
        ...
        ttk.Button(row, text="Generate CSV Templates", command=self._generate_csv_templates).pack(side=LEFT, padx=4)
        ttk.Button(row, text="Export Dataset (Excel)", command=self._export_dataset).pack(side=LEFT, padx=4)
        ttk.Checkbutton(row, text="Developer / Debug mode", variable=self.debug_mode).pack(side=LEFT, padx=16)
    ...
    def _generate_csv_templates(self):
        try:
            fund_master_cols = [
                "Fund Name", "Category", "NAV", "AUM_Cr", "Expense_Ratio_Pct",
                "Return_1Y_Pct", "CAGR_3Y_Pct", "CAGR_5Y_Pct", "Risk_Level",
                "Equity_Pct", "Debt_Pct", "Cash_Pct", "Benchmark", "Portfolio_Date"
            ]
            sample_fund_row = [
                "Sample Equity Growth Fund", "Equity - Large Cap Fund", "45.50", "2800", "1.75",
                "15.4", "18.2", "14.8", "Very High", "95.0", "3.0", "2.0", "Nifty 500 TRI", "2026-08-31"
            ]
            with open("template_fund_master.csv", "w", encoding="utf-8") as f:
                f.write(",".join(fund_master_cols) + "\n")
                f.write(",".join(sample_fund_row) + "\n")

            holdings_cols = ["Fund Name", "Company", "Sector", "Weight_Pct", "Portfolio_Date"]
            sample_holdings = [
                "Sample Equity Growth Fund,Company Alpha,Financial Services,8.5,2026-08-31",
                "Sample Equity Growth Fund,Company Beta,Information Technology,7.2,2026-08-31",
                "Sample Equity Growth Fund,Company Gamma,Healthcare,6.0,2026-08-31",
            ]
            with open("template_holdings.csv", "w", encoding="utf-8") as f:
                f.write(",".join(holdings_cols) + "\n")
                f.write("\n".join(sample_holdings) + "\n")

            hist_cols = ["Date", "Fund Name", "NAV"]
            sample_hist = [
                "2026-06-30,Sample Equity Growth Fund,42.10",
                "2026-07-31,Sample Equity Growth Fund,43.80",
                "2026-08-31,Sample Equity Growth Fund,45.50",
            ]
            with open("template_historical_nav.csv", "w", encoding="utf-8") as f:
                f.write(",".join(hist_cols) + "\n")
                f.write("\n".join(sample_hist) + "\n")

            messagebox.showinfo(
                "CSV Templates Created",
                "The following standard templates were created in the working directory:\n\n"
                "1. template_fund_master.csv (Fund Master Template)\n"
                "2. template_holdings.csv (Holdings & Sector Template)\n"
                "3. template_historical_nav.csv (Historical NAV Template)\n\n"
                "You can open, edit and populate these files, then import them anytime."
            )
        except Exception as exc:
            messagebox.showerror("Template Generation Error", f"Could not create templates: {exc}")
    ...
    def _import_holdings_data(self):
        ...
        try:
            ...
            col_map = {}
            for c in df.columns:
                cl = str(c).strip().lower().replace(" ", "_").replace("%", "pct")
                if cl in ["fund_name", "fund"]:
                    col_map[c] = "Fund Name"
                elif cl in ["company", "stock", "holding"]:
                    col_map[c] = "Company"
                elif cl in ["weight_pct", "weight", "weightpct"]:
            ...
            for fund_name in affected:
                ...
                for _, r in sub.iterrows():
                    ...
                    sector = str(r.get("Sector", "")).strip() if "Sector" in sub.columns and pd.notna(r.get("Sector")) else None
                    if sector:
                        COMPANY_SECTOR_MAP[company] = sector
                    else:
