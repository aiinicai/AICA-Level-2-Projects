...
def test_multipage_leading_txns_crop():
    # 1. Create Profile
    ...
    p2 = create_page(2, 60, 6, 6) # Starts at 60 (above 120 fallback) # Starts at 100 (outside page 1 region, should be caught by continuation)
    p3 = create_page(3, 60, 7, 12)
    ...
    p2.words.insert(0, RawWord("Continued Narration", 105, 195, 40, 50, 2))
    p2.word_count += 1
    ...
    # Explicitly check leading transactions on Page 2 and Page 3
    assert "Txn 6" in transactions[5].narration
    assert "Txn 7" in transactions[6].narration
    assert "Txn 12" in transactions[11].narration
    assert "Txn 13" in transactions[12].narration
