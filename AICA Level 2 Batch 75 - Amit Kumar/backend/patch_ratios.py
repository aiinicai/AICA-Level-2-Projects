...
# Note: Using IFNA or IFERROR to handle div by zero just in case
# Current Ratio (Row 5)
f_cr_cy = "='02_Balance_Sheet'!C36/'02_Balance_Sheet'!C20"
ws.cell(row=5, column=4, value=f_cr_cy)

# Debt Equity Ratio (Row 6)
f_de_cy = "=('02_Balance_Sheet'!C12+'02_Balance_Sheet'!C16)/('02_Balance_Sheet'!C8+'02_Balance_Sheet'!C9)"
ws.cell(row=6, column=4, value=f_de_cy)

# Net Profit Ratio (Row 7)
f_npr_cy = "='03_Profit_and_Loss'!C18 / '03_Profit_and_Loss'!C6"
ws.cell(row=7, column=4, value=f_npr_cy)

# ROE (Row 9)
f_roe_cy = "='03_Profit_and_Loss'!C18 / ('02_Balance_Sheet'!C8+'02_Balance_Sheet'!C9)"
ws.cell(row=9, column=4, value=f_roe_cy)

# Trade Receivable Days (Row 10)
f_trd_cy = "=('02_Balance_Sheet'!C32 / '03_Profit_and_Loss'!C6) * 365"
ws.cell(row=10, column=4, value=f_trd_cy)

# Trade Payable Days (Row 11)
f_tpd_cy = "=('02_Balance_Sheet'!C17 / '03_Profit_and_Loss'!C10) * 365"
ws.cell(row=11, column=4, value=f_tpd_cy)

# Inventory Days (Row 12)
f_invd_cy = "=('02_Balance_Sheet'!C31 / '03_Profit_and_Loss'!C10) * 365"
...
try:
    ...
    print(f"{'Ratio':<25} | {'Formula':<70} | {'Value'}")
    print("-" * 115)
    for name, f, val in ratios_to_print:
        ...
        if isinstance(val, (int, float)):
            ...
            if name in ["ROE", "Net Profit Ratio"]:
                v_str = f"{val*100:.2f}%"  # usually represented as percentage but formula doesn't have * 100
        else:
            v_str = str(val)
        print(f"{name:<25} | {f:<70} | {v_str}")
