...
- **A second instance of the `gdocs_script.js`-style build failure**, this time on the regular Chromium (not the headless shell) — `FileNotFoundError` on `PrivacySandboxAttestationsPreloaded\privacy-sandbox-attestations.dat`. Same root cause, different folder: this is one of Chrome's own auto-downloaded "component updater" folders (an advertising/privacy-sandbox attestation feature, unrelated to browser automation) that doesn't ship with the initial download — Chrome fetches it the first time it's actually launched with internet access, which happens naturally once you've tested the app. Because it arrives after the download, it's exactly the kind of file PyInstaller's Playwright hook sees during its scan but that can vanish before the actual copy step runs. Fixed at the time by deleting that specific folder before every build.
- **The exact same failure came back a third time — `gdocs_script.js` again, but now on the *regular* Chromium build, not the headless shell.** This made clear that deleting one specific offending file/folder after another was never actually fixing the underlying problem, just chasing symptoms: a different file breaks every time because every time the app is actually launched and used for real testing, Chrome's own background "component updater" silently adds, modifies, or removes various optional feature folders in the exact same Chromium copy PyInstaller is about to bundle — and PyInstaller's official Playwright hook records a file manifest when it scans that folder, then tries to copy those exact files moments later, failing if anything changed in between. Replaced the whack-a-mole file deletions with a proper fix: both build scripts now keep one pristine snapshot of Chromium taken right after download (before the app has ever been launched with real internet access), and restore from that snapshot immediately before every single build. Nothing touches the browser files between that restore and PyInstaller's scan+copy within the same script run, so they can never be a moving target again, regardless of how much real testing happens with the actual built app in between builds. Verified the restore logic directly: simulated Chrome's component-updater behavior (deleting a file, adding an unrelated folder) against a snapshot, confirmed the restore step produces an exact match to the original pristine state every time.
- **The exact same `gdocs_script.js` failure happened a FOURTH time — on the very first pristine snapshot creation itself**, proving the "changes between builds" theory wrong: this file vanished within a single, continuous PyInstaller run, with no time for anything to happen between builds at all. The much more likely explanation: real-time antivirus reacting to a script literally named "gdocs_helper" that looks like it injects into Google Docs — plausible given this environment's established history of security software interfering with this app (Smart App Control blocking the exe entirely, earlier). Rather than keep chasing whichever filename breaks next, both `.spec` files now filter `a.datas` immediately before `COLLECT` runs, dropping any data file that has vanished since it was scanned — checked as late as structurally possible within the `.spec` file to minimize the race window. This is a general fix: it doesn't matter which file disappears or why, since none of Chrome's optional UI/accessibility feature files are needed by an automation tool anyway. Verified directly against a simulated TOC list with real and "vanished" entries mixed together — confirmed only the genuinely missing ones get dropped.
- **The exact same failure happened a FIFTH time — `PrivacySandboxAttestationsPreloaded` again — on a completely clean, from-scratch build** (fresh venv, fresh Chromium download, fresh snapshot, no prior app usage at all). This proved the "check right before COLLECT" filter above still wasn't enough: the traceback showed the crash happening *inside* PyInstaller's own `assemble()` method, at the `shutil.copyfile()` call itself, meaning the file vanished in the narrow gap between that check and the actual copy a moment later — a race window so tight it strongly suggests antivirus actively deleting the file the moment something tries to read it, not just general background instability. Since any check-then-act approach has some race window no matter how small, the only way to fully close it is to handle the failure at the exact point it happens: both `.spec` files now monkey-patch `shutil.copyfile` itself (the exact function PyInstaller's packaging step calls) to catch `FileNotFoundError` and skip that one file with a clear log line, instead of letting the exception crash the entire build. Verified directly: confirmed normal file copies still work unchanged through the patched function, and a simulated vanished-file copy no longer raises and leaves no partial/corrupt file behind.
- **That patch worked exactly as intended (confirmed by its own "skipping a file that vanished" log line appearing) but the build still failed one line later** — PyInstaller's `assemble()` calls `os.chmod()` on the same destination path immediately after `copyfile()`, assuming the copy succeeded and something now exists there. Since the patched `copyfile` correctly declined to create anything for a missing source, `chmod` then failed the exact same way on the exact same path. Fixed by having the patch create an empty placeholder file at the destination when the source is missing, instead of leaving nothing there — satisfying whatever PyInstaller's own packaging step assumes exists afterward. Verified by reproducing the precise real-world sequence (`copyfile` then `chmod` on the identical path) end-to-end and confirming both steps now complete without error.
- **The root cause, finally actually confirmed — and every prior theory (antivirus, Chrome's background updater, timing races) was WRONG.** The placeholder fallback above *also* failed with the identical error, which was the clue that finally led somewhere real: measuring the actual failing path showed it was 266 characters long, past Windows' classic 260-character `MAX_PATH` limit. This project's own folder nesting (`...\Downloads\ClientLedgerIndia-Installer-Project\ClientLedgerIndia\...`) combined with PyInstaller's own `_internal\playwright\driver\package\.local-browsers\chromium-1234\chrome-win64\...` structure and Chromium's own deeply-nested optional feature folders was enough to push some resulting paths over the limit — at which point Win32's plain file APIs fail with "path not found" even though the path is completely valid. This explains everything observed across every previous attempt: which specific files failed (the ones nested deep enough to cross 260 characters), and why it looked inconsistent (different underlying Windows APIs handle long paths differently). The real fix: `\\?\` is Windows' own official long-path prefix, which tells Win32 to bypass the 260-character limit entirely. The `.spec` file now wraps `os.makedirs`, `shutil.copyfile`, and `os.chmod` to automatically apply this prefix to every path, so builds no longer depend on how deeply nested the folder someone extracts this project into happens to be. Verified the path-transformation logic directly (correct prefixing, no double-prefixing, correct UNC-path handling, zero effect on non-Windows platforms) and confirmed the patched functions still work correctly for normal file operations. `INSTALLATION-GUIDE.md` now also recommends extracting to a short path like `C:\CLI\` as extra insurance, since this fix covers this project's own file operations but not necessarily every tool in the chain (pip, Inno Setup, etc.) that a sufficiently long path could still affect.
- **Confirmed that concern was justified: the exact same 260-character limit then broke Inno Setup itself**, compiling the final installer from the very same long extraction path — a completely separate, native Windows program that the `.spec` file's Python-level monkey-patches have no way to reach. Inno Setup's compiler needs to read every source file to compress it, and a source path over 260 characters fails the same way PyInstaller's did. There's a documented community workaround (prefixing `SourceDir` with `\\?\`), but it's explicitly unverified even by the people who discussed it — one attempt at a related approach produced a different, equally confusing error ("No files found matching"). Rather than ship an unverified fix for a tool with no way to test it in this environment, the guidance was strengthened instead: `INSTALLATION-GUIDE.md` now states extracting to a short path (e.g. `C:\CLI\`) as a requirement, not a suggestion, since it's now confirmed to be the one fix that reliably resolves this for every tool in the build chain at once, not just PyInstaller.
- **RESOLVED, confirmed in a real build, not just in theory:** extracting the project to `C:\CLI\` instead of the long `Downloads\...\` path fixed both the PyInstaller build and the Inno Setup compile in the same session, with no further path-related errors of any kind. This closes out the whole path-length saga above (six separate failures across two tools, several wrong theories along the way) with an actual confirmed fix rather than another hypothesis. **Bottom line for anyone hitting a "path not found" / "cannot find the path specified" error anywhere in this build process: extract to a short path near the drive root and rebuild from there.**
...
**A one-time pristine snapshot of Chromium is kept inside `build_venv`** (`.local-browsers-pristine-backup`, another ~150–300MB), taken automatically the first time you build. Every later build restores Chromium from this snapshot before running PyInstaller, which is what keeps builds reliable even after you've actually run the app and Chrome's own background updater has quietly modified files inside it (see the changelog below for why this matters). This is normal and expected — nothing to clean up.
