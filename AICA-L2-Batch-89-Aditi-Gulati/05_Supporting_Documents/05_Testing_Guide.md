...
Expect **42 passed**. If pytest did not install, use:

```powershell
python tests\run_tests.py
```

which runs the same 42 tests with a small bundled substitute.
...
```
  ...
  Coverage 62% of the catalogue
  12 gaps, 2 partly met, 11 not assessed

  [x] advertising pixel
  [x] insecure form action
  [x] sensitive form fields
  [x] no policy page found
  [x] tracker loading without a notice naming it
  [x] working paper answers applied
