  ...
  try {
    ...
    if (existing.assignments?.length > 0) {
      const notifications = existing.assignments.map((assignment: { preparerId: string }) => ({
        ...
        message: `Compliance ${existing.complianceId} has been reviewed and is ready for preparation`,
        type: "APPROVAL",
        link: `/compliance/${id}`,
      }))
    ...
    console.error("POST /api/compliance/[id]/reviewer-approve error:", error)
