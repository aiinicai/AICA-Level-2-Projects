 ...
 */
export function recalculateITR(
  data: CompleteITRData,
  autoComputeTax: boolean = true,
  preserveManualTaxValues: boolean = false
): CompleteITRData {
  ...
  if (inc.salaryGross > 0 && inc.salaryStandardDeduction === 0) {
    inc.salaryStandardDeduction = p.assessmentYear >= '2025-26' || p.assessmentYear >= '2025' ? 75000 : 50000;
  }
  ...
  inc.capitalGainsNet =
    ...
    (inc.capitalGainsLTCG_12_5Pct || 0);

  // 5. Other Sources Net
  const otherGross =
    (inc.otherSourcesInterestSavings || 0) +
    (inc.otherSourcesInterestDeposits || 0) +
    (inc.otherSourcesDividends || 0) +
    (inc.otherSourcesFamilyPension || 0) +
    (inc.otherSourcesOthers || 0);
  inc.otherSourcesNet = Math.max(0, otherGross - (inc.otherSourcesDeductions || 0));
  ...
  const isNewRegime = (p.taxRegime || 'New Regime').includes('New');
  const allowedDeductions = isNewRegime ? (ded.sec80CCD2 || 0) : ded.totalDeductions;
  ...
  // Special Rate Incomes (Capital Gains)
  ...
  const specialTax =
    Math.round((inc.capitalGainsSTCG_15Pct || 0) * 0.15) +
    Math.round((inc.capitalGainsSTCG_20Pct || 0) * 0.20) +
    Math.round(Math.max(0, (inc.capitalGainsLTCG_10Pct || 0) - 100000) * 0.10) +
    Math.round(Math.max(0, (inc.capitalGainsLTCG_12_5Pct || 0) - 125000) * 0.125) +
    Math.round((inc.capitalGainsLTCG_20Pct || 0) * 0.20);
  tax.specialRateTax = specialTax;

  const normalTaxableIncome = Math.max(0, tax.totalTaxableIncome - specialRateIncome);

  // Auto Tax Computation: Always compute whenever taxOnTotalIncome is 0 or autoComputeTax is true
  if (autoComputeTax && (!preserveManualTaxValues || tax.taxOnTotalIncome === 0)) {
    ...
    // Preserve manual entry with accurate cascading
    tax.taxAfterRebate = Math.max(0, (tax.taxOnTotalIncome || 0) + (tax.specialRateTax || 0) - (tax.rebate87A || 0));
    if (tax.cess === 0 || autoComputeTax) {
      tax.cess = Math.round(((tax.taxAfterRebate || 0) + (tax.surcharge || 0)) * 0.04);
    }
    tax.grossTaxLiability = (tax.taxAfterRebate || 0) + (tax.surcharge || 0) + (tax.cess || 0);
  }

  tax.netTaxLiability = Math.max(0, (tax.grossTaxLiability || 0) - (tax.relief89 || 0) - (tax.relief90_91 || 0));
  tax.totalTaxAndInterest = (tax.netTaxLiability || 0) + (tax.interest234A || 0) + (tax.interest234B || 0) + (tax.interest234C || 0) + (tax.fee234F || 0);

  // Taxes Paid & Refund / Demand
  paid.totalTaxesPaid = (paid.advanceTax || 0) + (paid.tdsSalary || 0) + (paid.tdsNonSalary || 0) + (paid.tcs || 0) + (paid.selfAssessmentTax || 0);
...
export function parseITRFromText(rawText: string, fileName = 'ITR_Document.pdf'): CompleteITRData {
  ...
  // Tax Computation details from PDF if present
  base.taxComputation.taxOnTotalIncome = extractAmount([
    /(?:Tax\s*at\s*normal\s*rates|1a\s*Tax\s*at\s*normal\s*rates|Tax\s*on\s*total\s*income|Tax\s*Payable\s*on\s*Total\s*Income|Tax\s*payable\s*on\s*income\s*at\s*normal\s*rates)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.specialRateTax = extractAmount([
    /(?:Tax\s*at\s*special\s*rates|1b\s*Tax\s*at\s*special\s*rates|Special\s*rate\s*tax)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.rebate87A = extractAmount([
    /(?:Rebate\s*u\/s\s*87A|2\s*Rebate\s*u\/s\s*87A|Rebate\s*87A|Section\s*87A\s*Rebate)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.surcharge = extractAmount([
    /(?:Surcharge\s*after\s*marginal\s*relief|4\s*Surcharge|Surcharge)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.cess = extractAmount([
    /(?:Health\s*and\s*Education\s*Cess|Cess\s*@\s*4%|5\s*Health\s*and\s*Education\s*Cess|Health\s*&\s*Education\s*Cess)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.grossTaxLiability = extractAmount([
    /(?:Gross\s*tax\s*liability|6\s*Gross\s*tax\s*liability|Total\s*Tax\s*Payable)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.relief89 = extractAmount([
    /(?:Relief\s*u\/s\s*89|7\s*Relief\s*u\/s\s*89|Section\s*89\s*Relief|Relief\s*under\s*section\s*89)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.interest234A = extractAmount([
    /(?:Interest\s*u\/s\s*234A|9a\s*234A|Interest\s*234A)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.interest234B = extractAmount([
    /(?:Interest\s*u\/s\s*234B|9b\s*234B|Interest\s*234B)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.interest234C = extractAmount([
    /(?:Interest\s*u\/s\s*234C|9c\s*234C|Interest\s*234C)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.fee234F = extractAmount([
    /(?:Fee\s*u\/s\s*234F|Late\s*Fee\s*234F|9d\s*234F|Fee\s*for\s*default\s*in\s*furnishing)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);
  base.taxComputation.totalTaxAndInterest = extractAmount([
    /(?:Total\s*Tax,\s*Fee\s*and\s*Interest|10\s*Total\s*tax,\s*fee\s*and\s*interest|Total\s*Tax\s*and\s*Interest\s*Payable|Total\s*Tax\s*Liability)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i,
  ]);

  // Taxes Paid
  base.taxesPaid.advanceTax = extractAmount([/(?:Advance\s*Tax|11a\s*Advance\s*Tax)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i]);
  base.taxesPaid.tdsSalary = extractAmount([/(?:TDS\s*on\s*Salary|TDS\s*192|Schedule\s*TDS1|TDS\s*as\s*per\s*Form\s*16|11b_i\s*TDS\s*on\s*salary)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i]);
  base.taxesPaid.tdsNonSalary = extractAmount([/(?:TDS\s*on\s*other\s*than\s*Salary|TDS\s*Non-Salary|Schedule\s*TDS2|11b_ii\s*TDS\s*other)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i]);
  base.taxesPaid.tcs = extractAmount([/(?:TCS|Tax\s*Collected\s*at\s*Source|11c\s*TCS)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i]);
  base.taxesPaid.selfAssessmentTax = extractAmount([/(?:Self\s*Assessment\s*Tax|SAT|11d\s*Self\s*Assessment\s*Tax)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i]);
  base.taxesPaid.refundDue = extractAmount([/(?:Refund\s*Due|Refund\s*Claimed|Net\s*Refund|5\s*Refund|13\s*Refund)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i]);
  base.taxesPaid.taxPayable = extractAmount([/(?:Net\s*Tax\s*Payable|Amount\s*Payable|Balance\s*Tax\s*Payable|4\s*Net\s*tax\s*payable|12\s*Amount\s*payable)\s*[:\-]?\s*([₹Rs\.\d,\s]+)/i]);
...
export function parseITRFromJSON(jsonObj: any, fileName = 'ITR_Efiling.json'): CompleteITRData {
  ...
  const taxComp = root.PartB_TTI || root.TaxComputation || root.ScheduleTTI || {};
  ...
  // Tax Computation from JSON
  if (taxComp.TaxPayableOnTI?.TaxAtNormalRates) base.taxComputation.taxOnTotalIncome = parseIndianNumber(taxComp.TaxPayableOnTI.TaxAtNormalRates);
  if (taxComp.TaxPayableOnTI?.TaxAtSpecialRates) base.taxComputation.specialRateTax = parseIndianNumber(taxComp.TaxPayableOnTI.TaxAtSpecialRates);
  if (taxComp.Rebate87A || taxComp.Rebate) base.taxComputation.rebate87A = parseIndianNumber(taxComp.Rebate87A || taxComp.Rebate);
  if (taxComp.EducationCess || taxComp.HealthAndEduCess) base.taxComputation.cess = parseIndianNumber(taxComp.EducationCess || taxComp.HealthAndEduCess);
  if (taxComp.GrossTaxLiability) base.taxComputation.grossTaxLiability = parseIndianNumber(taxComp.GrossTaxLiability);
  if (taxComp.IntrstPay?.Intrst234A) base.taxComputation.interest234A = parseIndianNumber(taxComp.IntrstPay.Intrst234A);
  if (taxComp.IntrstPay?.Intrst234B) base.taxComputation.interest234B = parseIndianNumber(taxComp.IntrstPay.Intrst234B);
  if (taxComp.IntrstPay?.Intrst234C) base.taxComputation.interest234C = parseIndianNumber(taxComp.IntrstPay.Intrst234C);
  if (taxComp.IntrstPay?.LateFee234F) base.taxComputation.fee234F = parseIndianNumber(taxComp.IntrstPay.LateFee234F);
